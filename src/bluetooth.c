		...
		switch (hid_event.id) 
		{
			...
			{ /* Connection accepted event */
				if (is_controller(vid_pid)) 
				{
					...
					controllers[port].pid = vid_pid[1];
					controllers[port].vid = vid_pid[0];
...
{
	if(!(controllers[port].vid == MICROSOFT_VID && controllers[port].pid == XBOX_CONTROLLER_PID)) return;
	int leftX = 0x80;
	int leftY = 0x80;
	int rightX = 0x80;
	int rightY = 0x80;
